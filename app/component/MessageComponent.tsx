...
function MessageComponent({ message }: Props) {
  ...
  return (
    <div className={`flex w-fit space-x-3 ${isUser && "ml-auto"}`}>
      ...
      <div>
        <p
          className={`text-[0.65rem] px-[3px] pb-[2px] ${
            isUser ? "text-blue-500 text-right" : "text-gray-500 text-left"
          }`}
        >
          {message.username}
        </p>
        ...
        <div className="flex items-end">
          <div
            className={`px-3 py-2 rounded-lg w-fit ${
              isUser
                ? "text-white bg-blue-500 mr-auto order-2"
                : "text-gray-700 bg-gray-200 text-left"
            }`}
          >
            <p>{message.message}</p>
          </div>
